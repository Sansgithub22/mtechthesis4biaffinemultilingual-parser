...
def main():
    ...
    # ── Pre-compute frozen XLM-R embeddings (run XLM-R once, cache) ───────────
    print("\n[5] Pre-computing XLM-R embeddings (done once, reused every epoch) …")
    all_words_hi  = [s.words() for s in hi_sents]
    all_words_bho = [s.words() for s in bho_sents]
    cache_hi  = encoder.precompute_xlmr(all_words_hi,  desc="Hindi")
    cache_bho = encoder.precompute_xlmr(all_words_bho, desc="Bhojpuri")
